...
class SVDppMLP(nn.Module):
    ...
    def __init__(self, 
        ...
        if self.use_projector:
            # projector for implicit signal
            self.projector = nn.Sequential(nn.Linear(embedding_dim, embedding_dim))
        if use_gating:
    ...
    def forward(self, 
        ...
        sqrt_lengths =  (implicit_lengths.sqrt() + 1e-9) # (batch_size,)
        norm_term = (1.0 / sqrt_lengths).unsqueeze(-1) # (batch_size, 1)
        y_sum = y_sum * norm_term # (batch_size, embedding_dim)

        # project implicit signal to the same space as scientist
        if self.use_projector:
            y_sum = self.projector(y_sum)

        # gating to combine scientist factor and transformed implicit factor
        if self.use_gating:
            gate_input = torch.cat([p_s, y_sum], dim=-1) # (batch_size, 2*embedding_dim)
            gate = torch.sigmoid(self.gate(gate_input)) # (batch_size, 1)
            user_representation = gate * p_s + (1 - gate) * y_sum # (batch_size, embedding_dim)
        else:
            user_representation = p_s + y_sum
...
class SVDppAG(nn.Module):
    ...
    def forward(self, 
        ...
        if self.use_attention:
            ...
            # scale the implicit signal by the square root of the number of implicit items
            processed_implicit_signal = processed_implicit_signal * (implicit_lengths.float().sqrt() + 1e-9).unsqueeze(-1)
        else:
